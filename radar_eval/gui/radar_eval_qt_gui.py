...
class SYRA_MAIN_GUI(QtWidgets.QMainWindow):
    ...
    def _plot_spectrograms(self) -> None:
        ...
        for plot_counter, channel in enumerate(self.plot_channel_order_list):
            if plot_counter <= 3:
                self._update_spectrogram_plot_line(channel, 'Channel 1', plot_counter)
            elif plot_counter > 3 and plot_counter < 8:
                self._update_spectrogram_plot_line(channel, 'Channel 1', plot_counter - 4)

    def _update_spectrogram_plot_line(self, channel_name: str, channel: str, plot_counter: int) -> None:
        """
        Helper function to update the spectrogram plot line for a specific channel.
        """
        self.gui_controller.syra_plotter.spectrogram.plotlines[f'{self.gui_controller.tab_name_main_instance_window} {channel_name}'].setImage(
    ...
    def _plot_range_dopplers(self) -> None:
        ...
        for plot_counter, channel in enumerate(self.plot_channel_order_list):
            if plot_counter <= 3:
                self._update_range_doppler_plot_line(channel, 'Channel 1', plot_counter)
            elif plot_counter > 3 and plot_counter < 8:
