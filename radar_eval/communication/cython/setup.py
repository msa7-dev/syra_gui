...
module_name = "radar_bridge_mcu_commands"

# Path to the Cython file
cython_file = f"./radar_eval/communication/cython/{module_name}.pyx"
output_dir = "./radar_eval/communication/cython/"  
...
setup(
    name="radar_bridge_mcu_commands",
    ext_modules=cythonize(ext_modules, compiler_directives={'language_level': '3'}),
    include_dirs=[numpy_include_path],  # Convert to string here
    zip_safe=False,
)
# Search for files matching the pattern
files_to_move = glob.glob("./radar_bridge_mcu_commands.*")

# Move each found file to the desired directory
for file_to_move in files_to_move:
    destination_path = "./radar_eval/communication/cython"
