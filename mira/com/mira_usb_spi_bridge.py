...
class MIRA_USB_SPI_BRIDGE():
    def __init__(self, radar_param: MIRA6024_RADAR_PARAMETER) -> None:
        ...
        if self.device is None:
            return
        logger.debug('Finished USB Device Init')
    ...
    def spi_init_bgt(self) -> None:
        ...
        try:
            ...
            sys.exit("Error writing USB data: " + str(e))
    ...
    def spi_bgt_finished_init(self) -> None:
        self.usb_device_available.get_lock().acquire(block=True)
        self.endpoint_out.write(bytearray([self.stm_commands.init_finished_cmd]))
        usb.util.release_interface(self.device, self.interface)
        self.usb_device_available.get_lock().release()
        
    def spi_bgt_reset(self) -> None:
        self.usb_device_available.get_lock().acquire(block=True, timeout=10)
        self.endpoint_out.write(bytearray([self.stm_commands.bgt_reset_cmd]))
        usb.util.release_interface(self.device, self.interface)
        self.usb_device_available.get_lock().release()
        
    def spi_stm_reset(self) -> None:
        self.usb_device_available.get_lock().acquire(block=True, timeout=10)
        self.endpoint_out.write(bytearray([self.stm_commands.stm_rst_cmd]))
        usb.util.release_interface(self.device, self.interface)
        self.usb_device_available.get_lock().release()
        self.init_stm = False
    ...
    def raw_data_aquisition_process(self, raw_data_queue: multiprocessing.Queue,
        ...
        DATA_ALLOCATION = np.uint32(USB_SPI_BRIDGE_DATA_ALLOCATION+self.n_fifo_overhead*9)
    ...
    def spi_read_reg(self, reg_adr: np.uint8) -> list[np.uint8, np.ndarray]:
        ...
        try:
            self.usb_device_available.get_lock().acquire(block=True)
            ret_payload = self.endpoint_in.read(4)
            usb.util.release_interface(self.device, self.interface)
            self.usb_device_available.get_lock().release()
        except usb.core.USBError as e:
            sys.exit("Error reading USB data: " + str(e))
    ...
    def spi_read_n_reg(self, reg_start_adr: np.uint8, n_reg: np.uint16) -> np.ndarray:
        ...
        try:
            self.usb_device_available.get_lock().acquire(block=True)
            ret_payload = self.endpoint_in.read(n_reg*3)
            usb.util.release_interface(self.device, self.interface)
            self.usb_device_available.get_lock().release()
        except usb.core.USBError as e:
            sys.exit("Error reading USB data: " + str(e))
    ...
    def init_stm_usb_device(self) -> None:
        ...
        try:
            ...
            sys.exit(f"Error finding USB device: {str(usb_error)}") 
        ...
        try:
            ...
            sys.exit(f"Could not retrieve USB Device Descriptor.")
