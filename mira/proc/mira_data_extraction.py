...
from mira.save.mira_save_meas import MIRA6024_SAVE_MEAS
...
class MIRA6024_DATA_EXTRACTOR():
    def __init__(self, mira_device):
        ...
        if self.radar_param.meas.measurement_flag == True:
            self.save_meas = MIRA6024_SAVE_MEAS(mira_device)
    ...
    def data_extracting_process(self, raw_fifo_data_queue: multiprocessing.Queue,
        ...
        radar_data_cube_build_buffer = np.zeros((self.radar_param.sys.n_samples_per_chirp[0], # Dim 1 - Samples
                                                4,#  int(sum(self.radar_param.sys.rx_active_antennas)/2), # Dim. 2 - Number RX Antennas
                                                2, #sum(self.radar_param.sys.tx_active_antennas), # Dim. 3 - Number Chirps per Shape
                                                self.radar_param.sys.shape_set_repetition,
        ...
        self.package_chirp_header_index_distance = np.uint32(
            ...
            (self.radar_param.sys.rx_active_antennas[0]*0 + 4 / 2) * 3 + 9) # TODO 4
        ...
        while not stop_event.is_set():
            ...
            for header_match in header_matches:
                ...
                if np.all(radar_data_cube_build_buffer[:,:, self.header_dict['cs'], 
                          np.uint16(self.header_dict['shape_grp_cnt']/2),
                          frame_cnt] == 0):
